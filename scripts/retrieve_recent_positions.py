"""
Binance Position History Retrieval Script - Last 2 Weeks
Fetches positions from May 17, 2026 onwards
"""
import sys
import os
import json
...
def main():
    """Main execution function - Fetches last 2 weeks"""
    print(f"\n{'='*80}")
    print(f"BINANCE POSITION HISTORY - LAST 2 WEEKS")
    print(f"{'='*80}\n")

    # Define start time: May 17, 2026, 12:00 AM CST (2 weeks ago from May 31)
    START_YEAR = 2026
    START_MONTH = 5
    START_DAY = 17
    START_HOUR = 0
    ...
    try:
        ...
        csv_path = os.path.join(output_dir, f"positions_last_2weeks.csv")
        json_path = os.path.join(output_dir, f"positions_last_2weeks.json")
        md_path = os.path.join(output_dir, f"positions_last_2weeks.md")
        ...
        generate_position_summary(positions, md_path, f"Last 2 Weeks ({date_str} onwards)")
