...
- After 1st hour: If no profit → EXIT
- After 1st hour: If profit → CONTINUE
- Check every subsequent hour: If still profitable → CONTINUE, else EXIT
- Max timeout: Still cap at 4 hours or until TP hit
"""
...
HOURLY_CHECK_MIN = 60   # Check every 60 minutes
...
def simulate_trade_with_hourly_check(klines, entry_idx, entry_price, direction, tp, sl):
    ...
    Logic:
    - Check at 60 min mark: if no profit → EXIT
    - If profitable at 60 min → continue to 120 min
    - Check at 120 min: if no profit → EXIT
    - If profitable at 120 min → continue to 180 min
    - Check at 180 min: if no profit → EXIT
    - If profitable at 180 min → continue to 240 min (hard cap)
    """
    max_hold = min(MAX_TIMEOUT_MIN, len(klines) - entry_idx - 1)

    # Track hourly checkpoints
    hourly_checkpoints = [60, 120, 180, 240]  # Minutes
...
def main():
    """Main execution"""
    print("="*80)
    print("HOURLY PROFIT CHECK STRATEGY VALIDATION")
    print("="*80)
    print()
    ...
    print(f"  Hourly Check: Every {HOURLY_CHECK_MIN} min")
    print(f"  → If NO PROFIT at checkpoint → EXIT immediately")
    print(f"  → If PROFITABLE at checkpoint → CONTINUE to next hour")
    print(f"  Max Timeout: {MAX_TIMEOUT_MIN} min")
    print()
    ...
    # Test strategy
    print(f"Testing HOURLY PROFIT CHECK strategy:")
    print("-"*80)
    ...
    for i, mover in enumerate(movers, 1):
        ...
        for idx in range(20, len(klines) - MAX_TIMEOUT_MIN):
            ...
            if direction:
                # Simulate trade with hourly checks
    ...
    # Results
    print("\n" + "="*80)
    print("RESULTS WITH HOURLY PROFIT CHECK")
    print("="*80)
    ...
    print(f"Hourly exits: {hourly_exits}/{total_entries} ({hourly_exits/total_entries*100:.1f}%)")
    ...
    # Compare to original strategy (240 min timeout, no hourly checks)
    print("\n" + "="*80)
    print("COMPARISON: Original vs Hourly Check Strategy")
    print("="*80)
    print(f"                          Original (240min)    Hourly Check")
    print(f"Avg Hold Time:            218.5 min            {avg_hold:.1f} min")
    print(f"Win Rate:                 51.8%                {wins/total_entries*100:.1f}%")
    print(f"TP Hit Rate:              14.3%                {tp_hits/total_entries*100:.1f}%")
    print(f"Early Exits:              0                    {hourly_exits}")
    print(f"Net P&L:                  $13.40               ${total_pnl:.2f}")
    print(f"ROI:                      +13.4%               {total_pnl:+.1f}%")

    # Save results
    analysis_dir = Path(__file__).parent
    output_file = analysis_dir / 'hourly_check_results.json'
