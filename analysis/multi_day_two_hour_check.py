#!/usr/bin/env python3
"""
Multi-Day Validation with 2-Hour Profit Check

Test 2-hour check strategy across multiple periods
"""
...
def main():
    """Main execution"""
    print("="*80)
    print("MULTI-DAY VALIDATION WITH 2-HOUR PROFIT CHECK")
    print("="*80)
    print()
    ...
    print(f"  Exit if no profit at 2h/4h checkpoints")
    print()
    ...
    for period in validation_periods:
        ...
        if results:
            ...
            print(f"    2-hour exits: {results['check_exit_rate']:.1f}%")
    ...
    # Summary
    print("\n" + "="*80)
    print("SUMMARY - 2-HOUR CHECK STRATEGY ACROSS ALL PERIODS")
    print("="*80)
    ...
    print()
    print(f"{'Period':<30} {'Coverage':>10} {'Win%':>8} {'TP%':>7} {'2h%':>8} {'P&L':>10} {'ROI':>8}")
    print("-"*80)
    ...
    for result in all_results:
        ...
        print(f"{result['period']:<30} {r['coverage']:>9.1f}% {r['win_rate']:>7.1f}% {r['tp_hit_rate']:>6.1f}% {r['check_exit_rate']:>7.1f}% ${r['net_pnl']:>8.2f} {r['roi_pct']:>7.1f}%")

    print("-"*80)
    print(f"{'AVERAGE':<30} {sum(r['results']['coverage'] for r in all_results)/len(all_results):>9.1f}% {total_wins/total_trades*100:>7.1f}% {sum(r['results']['tp_hit_rate'] for r in all_results)/len(all_results):>6.1f}% {sum(r['results']['check_exit_rate'] for r in all_results)/len(all_results):>7.1f}% ${total_pnl:>8.2f} {total_pnl:>7.1f}%")
    print()
    ...
    output_file = analysis_dir / 'multi_day_two_hour_check_results.json'
